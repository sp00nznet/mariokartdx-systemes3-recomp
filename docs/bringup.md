# Bringing up Mario Kart Arcade GP DX

How this went from a PE that would not open a window to a race won on a
gamepad, in the order it happened. The wrong turns are kept: in a static
recompilation the interesting failures are almost never the CPU, and a
narrative that only lists the fixes is not much use to the next person.

Split out of the README, which had grown to 686 lines.
